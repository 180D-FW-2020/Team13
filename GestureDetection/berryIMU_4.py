...
#Kalman filter variables
    ...
    #shoot = 0 means stop shooting
        ...
        if directionChecked <= 0:
            preaccz = abs(8500*math.cos(math.radians(kalmanX))*math.cos(math.radians(kalmanY)))
            constCosXY = abs(math.cos(math.radians(kalmanX))*math.cos(math.radians(kalmanY)))
            constSinx = abs(math.sin(math.radians(kalmanY)))
            constCosx = abs(math.cos(math.radians(kalmanY)))
            preaccx = abs(8500*constSinx)
          
            if ((ACCz - preaccz) <= ((6000-8500)*constCosXY)):
                
                direction[sstime] = 1 #up

            elif((ACCz-preaccz) >= ((9800-8500)*constCosXY)):
                direction[sstime] = 2 #down
            if ACCx < 0:
                if ((ACCx+preaccx) < (-800*abs(constCosx))):
                    direction[sstime] = 3 #left
                elif ((ACCx+preaccx) > (700*abs(constCosx)) ):
                    direction[sstime] = 4 #right
            elif ACCx > 0:
                if ((ACCx-preaccx) < (-800*abs(constCosx))):
                    direction[sstime] = 3 #left
                elif ((ACCx-preaccx) > (700*abs(constCosx)) ):
            ...
            if(sstime == dirRange-1):
                for n in range (dirRange-1):
