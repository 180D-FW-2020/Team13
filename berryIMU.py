...
while True:
    ...
    gyroXYZ = [GYRx, GYRy, GYRz]
    madgwicAngles = MadgwickAHRS()
    madgwicAngles.update_imu(gyroXYZ, accXYZ)
    madgwick = madgwicAngles.quaternion.to_angle_axis()
    ...
    if 1:                       #Change to '0' to stop  showing the angles from the Kalman filter
       # outputString +="# madgwickX %5.2f   madgwickY %5.2f #" % ((madgwick[0]/(2 * M_PI))*360,(madgwick[1]/(2*M_PI))*360)
        outputString +="# kalmanX %5.2f   kalmanY %5.2f #" % (kalmanX,kalmanY)
        if kalmanX >= 45:
            outputString += " foward direction "
        elif kalmanX <= -35:
            outputString += " backward direction "
        else:
            outputString += " unknown direction "
    print(outputString)
